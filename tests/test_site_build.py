...
def test_build_preserves_older_releases_and_allowlisted_downloads(source, tmp_path):
    ...
    fifth = distribution(tmp_path / "fifth", "2.2.0", "1" * 40)
    output = tmp_path / "public"
    site_build.build(current, output, "a" * 40, True,
                     archive_distribution=[first, second, third, fourth, fifth],
                     archive_source=["b" * 40, "c" * 40, "e" * 40, "f" * 40, "1" * 40])
    manifest = json.loads((output / "site-manifest.json").read_text())
    assert manifest["docs_source_clean"] is False
    assert [item["version"] for item in manifest["archived_distributions"]] == ["1.0.0", "1.1.0", "2.0.0", "2.1.0", "2.2.0"]
    for directory, version in ((current, CURRENT_VERSION), (first, "1.0.0"), (second, "1.1.0"), (third, "2.0.0"), (fourth, "2.1.0"), (fifth, "2.2.0")):

...
-- Existing copy has no recorded generation context. Leave it unknown rather
-- than claiming that today's source facts were used to prepare older drafts.
ALTER TABLE drafts ADD COLUMN IF NOT EXISTS generation_source_fingerprint
    TEXT CHECK (generation_source_fingerprint ~ '^[a-f0-9]{64}$');
ALTER TABLE drafts ADD COLUMN IF NOT EXISTS reviewed_source_fingerprint
    TEXT CHECK (reviewed_source_fingerprint ~ '^[a-f0-9]{64}$');

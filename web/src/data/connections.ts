...
export function engineRequestOptions(access: ReviewAccess, baseUrl = ''): RequestInit {
  if (baseUrl && isLocalSession(access)) throw new Error('Remote engines require their own access token. Your local browser session cannot be forwarded.')
...
export async function verifyEngineAccess(baseUrl: string, token: string): Promise<void> {
  ...
  try {
    response = await fetch(`${baseUrl}/v1/ui/setup`, {

...
export function createHttpClient(baseUrl = '', operatorToken = ''): ReviewClient {
  ...
  return {
    ...
    listRecords: (offset = 0) => request<RecordPage>(`/records?limit=50&offset=${Math.max(0, Math.floor(offset))}`),
    ...
    listInbox: () => request<InboxResult>('/inbox?limit=200'),
    async listOutbox() {
